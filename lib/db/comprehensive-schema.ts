...
export const achievements = pgTable('achievements', {
  ...
  criteria: json('criteria').notNull(), // Requirements to earn this achievement
  pointsAwarded: integer('points_awarded').notNull(),
  isActive: boolean('is_active').default(true),
  rarityLevel: varchar('rarity_level', { length: 20 }).default('common'), // 'common', 'rare', 'epic', 'legendary'
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow(),
});
...
export const userAchievements = pgTable('user_achievements', {
  ...
  progress: json('progress'), // Current progress towards achievement
  isCompleted: boolean('is_completed').default(true),
  notificationSent: boolean('notification_sent').default(false),
  createdAt: timestamp('created_at').defaultNow(),
}, (table) => ({
  // Composite unique constraint: one achievement per user (can't earn same achievement twice)
